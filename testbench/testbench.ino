...
void setup() {
    ...
    Serial.println("setint - Enter intensity level (0-15)");
    Serial.println("Valid ranges - Row: 0-7 (7 is top), Column: 0-31");

    // Initialize the display
    mx.begin();
    mx.control(MD_MAX72XX::INTENSITY, 2);  // Default intensity
...
void loop() {
    ...
    if (Serial.available() > 0) {
        ...
        if (input == "indoff" || input == "rowoff" || input == "coloff" || 
            input == "indon" || input == "rowon" || input == "colon" || 
            input == "setint") {
            command = input;
            if (command == "setint") {
                Serial.println("Enter intensity level (0-15):");
            } else {
        ...
        if (command == "setint") {
            ...
            if (intensity >= 0 && intensity <= 15) {
                ...
                Serial.println("Invalid intensity! Enter a value between 0 and 15.");
            }
        } else if (command == "indoff") {
            ...
            Serial.println("Unknown command! Use 'indoff', 'rowoff', 'coloff', 'setint', etc.");
        }
