function initiate_image_processing(ref_img, cam)
    ...
    % Updated: Create masks using saturation
    ref_saturation = ref_hsv(:, :, 2);
    test_saturation = test_hsv(:, :, 2);
    saturation_threshold = 0.2;

    ref_mask = (ref_saturation < saturation_threshold); % Low saturation in reference image
    test_mask = (test_saturation < saturation_threshold); % Low saturation in test image
