...
try
    ...
    temp_figure_paths = cell(5,1);
    for fig_num = 1:5
        temp_figure_paths{fig_num} = fullfile(temp_dir, sprintf('temp_figure_%d.png', fig_num));
        figure(fig_num);
        print(gcf, temp_figure_paths{fig_num}, '-dpng', '-r300');
        close(fig_num);
    end
    ...
    vis_sec = Section('Visualizations');

    % Add each figure with appropriate captions
    figure_titles = {
        'Reference LED Mask',
        'Test LED Mask',
        'Reference Mask with Bounding Boxes',
        'Missing LEDs Highlighted',
        'LED Intensity Ratios'
    };

    for i = 1:5
        fig_para = Paragraph(sprintf('Figure %d: %s', i, figure_titles{i}));
        add(vis_sec, fig_para);

        img = Image(temp_figure_paths{i});
        img.Style = {Width('6in')};
        add(vis_sec, img);
